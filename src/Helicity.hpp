...
namespace LCS
{
    ...
    template <class Type>
    class Helicity : public field<double>
    {

        private:

            int sgn(Type val);                              // For determining signs
            void _computeHelicityAuxiliaryGrid();           // Compute using the aux. grid
            void _computeHelicityFiniteDifferencing();

        public:

            /* Class constructor */

            Helicity(position<double>& pos);
